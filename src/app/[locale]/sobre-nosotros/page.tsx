...
export default async function SobreNosotrosPage({
  ...
  const stats = [
    ...
    { value: t('stats.clients'),  icon: 'groups',       num: 20 },
    { value: t('stats.countries'),icon: 'public',       num: 3  },
  ];
    ...
    <>
      ...
      <section className="px-8 py-20 mx-4 md:mx-8 bg-surface-container-low rounded-[2rem]">
        <div className="max-w-5xl mx-auto text-center">
          ...
          {/* Stats */}
          <Stagger className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-12">
            {stats.map(({ value, icon, num }) => (
              <StaggerItem key={value}>
                <ScaleOnHover>
                  <div className="p-8 bg-white rounded-[1.5rem] border-b-4 border-primary shadow-sm flex flex-col items-center gap-3">
                    ...
                    <div className="text-4xl font-extrabold text-primary font-headline">
                      <AnimatedCounter to={num} suffix="+" />
                    </div>
                    <div className="text-sm text-on-surface-variant font-label">{value}</div>
                  </div>
                </ScaleOnHover>
              </StaggerItem>
            ))}
      ...
      <section className="px-8 py-32 max-w-7xl mx-auto">
        ...
        <Stagger className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {values.map(({ key, icon, bg, color }) => (
            <StaggerItem key={key}>
              <ScaleOnHover>
                <div className="group p-10 bg-surface-container rounded-[2rem] hover:bg-surface-container-high transition-all duration-500 cursor-default h-full">
                  <div
